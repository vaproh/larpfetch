...
| `--disk-info [home\|physical\|all\|PATH]` | Per-disk breakdown. `home` (default) = disk holding `$HOME`, `physical` = all real disks, `all` = include virtual mounts (tmpfs, proc, …), `PATH` = that specific disk |
